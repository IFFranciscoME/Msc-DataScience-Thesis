...
def g_ohlc(p_ohlc, p_theme, p_dims, p_labels, p_vlines):
    ...
    fig_g_ohlc.update_layout(
        xaxis=dict(titlefont=dict(color=p_theme['p_theme']['color_1']),
                   tickfont=dict(color=p_theme['p_theme']['color_1'],
                                 size=p_theme['p_theme']['font_axis'])),
        yaxis=dict(zeroline=False, automargin=True,
                   titlefont=dict(color=p_theme['p_theme']['color_1']),
                   tickfont=dict(color=p_theme['p_theme']['color_1'],
                                 size=p_theme['p_theme']['font_axis']),
                   showgrid=True))
    ...
    if p_vlines is not None:
        ...
        shapes_list = list()
        for i in p_vlines:
            shapes_list.append({'type': 'line', 'fillcolor': p_theme['p_theme']['color_1'],
                                'line': {'color': p_theme['p_theme']['color_1'], 'dash': 'dashdot'},
                                'x0': i, 'x1': i, 'xref': 'x',
    ...
    # Update layout for the background
    fig_g_ohlc.update_layout(title_font_size=p_theme['p_theme']['font_title'],
                             title=dict(x=0.5,
                                        text='Grafica 1:' + '<b> ' + p_theme['p_labels']['title'] + ' </b>'),
                             yaxis=dict(titlefont=dict(size=p_theme['p_theme']['font_axis'] + 4)),
...
def g_relative_bars(p_x, p_y0, p_y1, p_theme, p_dims):
    ...
    # Update layout for the y axis
    fig_relative_bars.update_yaxes(showgrid=False, range=[-1.05, 1])
    ...
    # Update layout for the background
    fig_relative_bars.update_layout(title_font_size=p_theme['p_theme']['font_title'],
                                    title=dict(x=0.5,
                                               text='Grafica 3:' + '<b> ' +
